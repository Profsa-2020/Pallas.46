...
<?php
     ...
     if (isset($_SERVER['HTTP_REFERER']) == true) {
          if (limpa_pro($_SESSION['wrknompro']) != limpa_pro($_SERVER['HTTP_REFERER'])) {
               ...
               $ret = gravar_log(6,"Entrada na página de manutenção de clientes do sistema Pallas.46 - SearchMidia");  
          }
     }
...
<body id="box00">
     <h1 class="cab-0">Manutenção de Clientes - SearchMidia - Profsa Informática</h1>
     <div class="row">
          <div class="col-12">
               ...
               <form class="tel-1" name="frmTelMan" action="" method="POST">
                    ...
                    <div class="row">
                         <div class="col-md-4">
                              ...
                              <input type="text" class="form-control" maxlength="50" id="aut" name="aut"
                                   value="<?php echo $aut; ?>" />
                         </div>
                         <div class="col-md-4">
                              <label>Cargo</label>
...
function ler_cliente(&$cha, &$cgc, &$des, &$fan, &$ema, &$sta, &$tel, &$cep, &$end, &$num, &$com, &$bai, &$cid, &$est, &$ins, &$cel, &$con, &$sit, &$pes, &$aut, &$cpf, &$car, &$gru, &$ram, &$obs) {
     ...
     if ($nro == 0 || $reg == false) {
          echo '<script>alert("Código da cliente informada não cadastrada");</script>';
          $nro = 1;
     }else{
          $cha = $reg['idcliente'];
          $des = $reg['clirazao'];
          ...
          $cep = $reg['clicep'];
          $end = $reg['cliendereco'];
          $num = $reg['clinumeroend'];
          $com = $reg['clicomplemento'];
          $bai = $reg['clibairro'];
          $cid = $reg['clicidade'];
          $est = $reg['cliestado'];
          $pes = $reg['clipessoa'];
...
function consiste_cli() {
     ...
     if (trim($_REQUEST['ema']) == "") {
          echo '<script>alert("E-mail da cliente não pode estar em branco");</script>';
          return 3;
     }
...
function incluir_cli() {
 ...
 function alterar_cli() {
     ...
     $sql .= "clicnpj = '". limpa_nro($_REQUEST['cgc']) . "', ";
     $sql .= "clistatus = '". $_REQUEST['sta'] . "', ";
     $sql .= "cliinscricao = '". $_REQUEST['ins'] . "', ";
     $sql .= "clirazao = '". $_REQUEST['des'] . "', ";
     $sql .= "clifantasia = '". $_REQUEST['fan'] . "', ";
     $sql .= "clicep = '". limpa_nro($_REQUEST['cep']) . "', ";
     $sql .= "cliendereco = '". $_REQUEST['end'] . "', ";
     $sql .= "clinumeroend = '". limpa_nro($_REQUEST['num']) . "', ";
     $sql .= "clicomplemento = '". $_REQUEST['com'] . "', ";
     $sql .= "clibairro = '". $_REQUEST['bai'] . "', ";
     $sql .= "clicidade = '". $_REQUEST['cid'] . "', ";
     $sql .= "cliestado = '". $_REQUEST['est'] . "', ";
     $sql .= "clitelefone = '". $_REQUEST['tel'] . "', ";
     $sql .= "clicelular = '". $_REQUEST['cel'] . "', ";
     $sql .= "clicontato =  '". $_REQUEST['con'] . "', ";
     $sql .= "cliemail = '". $_REQUEST['ema'] . "', ";
     $sql .= "clisite = '". $_REQUEST['sit'] . "', ";
     $sql .= "cligrupo = '". $_REQUEST['gru'] . "', ";
     $sql .= "cliautorizante = '". $_REQUEST['aut'] . "', ";
