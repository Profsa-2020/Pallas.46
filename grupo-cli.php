...
<head>
     ...
     <title>Grupo de Cliente - Gerenciamento de Contratos - SearchMidia - Profsa Informátda Ltda</title>
</head>
...
$(document).ready(function() {
     ...
     $(".exc").click(function() {
          ...
          let res = confirm('Confirma exclusão de grupo de cliente informado em opção ?');
          if (res == true) {
     ...
     $('#tab-0').DataTable({
          ...
          "language": {
               ...
               "infoEmpty": "Sem registros de Grupos de Clientes ...",
               "sSearch": "Buscar:",
...
<?php
     ...
     if (isset($_SERVER['HTTP_REFERER']) == true) {
          if (limpa_pro($_SESSION['wrknompro']) != limpa_pro($_SERVER['HTTP_REFERER'])) {
               ...
               $ret = gravar_log(6,"Entrada na página de manutenção de grupo de cliente do sistema Pallas.46 - SearchMidia");  
          }
     }
     ...
     if ($_SESSION['wrkopereg'] == 3) { 
          $bot = 'Deletar'; 
          $del = "cor-2";
          $per = ' onclick="return confirm(\'Confirma exclusão de Grupo de Cliente informado em tela ?\')" ';
     }
 ...
 if (isset($_REQUEST['salvar']) == true) {
      if ($_SESSION['wrkopereg'] == 1) {
           ...
           if ($sta == 0) {
                $ret = incluir_gru();
                $cod = ultimo_cod();
                $ret = gravar_log(11,"Inclusão de novo Grupo de Cliente de campo: " . $des); 
      ...
      if ($_SESSION['wrkopereg'] == 2) {
           ...
           if ($sta == 0) {
                $ret = alterar_gru();
                $cod = ultimo_cod(); 
                $ret = gravar_log(12,"Alteração de Grupo de Cliente cadastrado: " . $des); 
                $des = ''; $obs = ''; $sta = 00;  $_SESSION['wrkopereg'] = 1; $_SESSION['wrkcodreg'] = 0;
           }
      }
      if ($_SESSION['wrkopereg'] == 3) {
           ...
           $cod = ultimo_cod(); 
           $ret = gravar_log(13,"Exclusão de Grupo de Cliente cadastrado: " . $des); 
           $des = ''; $obs = ''; $sta = 00;  $_SESSION['wrkopereg'] = 1; $_SESSION['wrkcodreg'] = 0;
      }
}
?>
...
<body id="box00">
     <h1 class="cab-0">Manutenção de Grupo de Cliente - SearchMidia - Profsa Informática</h1>
     <div class="row">
          <div class="col-md-12">
          ...
          <div class="qua-0">
               <div class="row qua-2">
                    <div class="col-md-10 text-left">
                         <span>Manutenção de Grupos de Cliente</span>
                    </div>
                         ...
                         <form name="frmTelNov" action="man-grupo.php?ope=1&cod=0" method="POST">
                              <div class="text-center">
                                   <button type="submit" class="bot-4" id="nov" name="novo"
                                        title="Mostra campos para criar novo Grupo de Cliente no sistema">Adicionar</button>
                              </div>
                         </form>
                    </div>
               </div>
               <form class="tel-1" name="frmTelMan" action="" method="POST">
                    <div class="row">
                         ...
                         <div class="col-md-8">
                              <label>Descrição do Grupo de Cliente</label>
...
if ($_SESSION['wrkopereg'] == 1 && $_SESSION['wrkcodreg'] == $cod) {
     exit('<script>location.href = "grupo-cli.php?ope=1&cod=0"</script>');
}
...
function consiste_gru() {
     ...
     if (trim($_REQUEST['des']) == "") {
          echo '<script>alert("Descrição do Grupo de Cliente não pode estar em branco");</script>';
          return 1;
     }
...
function carrega_gru() {
     include_once "dados.php";
     $com = "Select * from tb_grupo where grutiporeg = 1 and gruempresa = " . $_SESSION['wrkcodemp'] . " order by grudescricao, idgrupo";
     $nro = leitura_reg($com, $reg);
     foreach ($reg as $lin) {
          $txt =  '<tr>';
          $txt .= '<td class="text-center"><a href="grupo-cli.php?ope=2&cod=' . $lin['idgrupo'] . '" title="Efetua alteração do registro informado na linha"><i class="large material-icons">create</i></a></td>';
          $txt .= '<td>' . '<button type="button" class="exc bot-2 cor-1" name="exc" cod="' . $lin['idgrupo'] . '" title="Efetua exclusão do registro informado na linha"><i class="cor-1 large material-icons">delete_forever</i></button>' . '</td>';
...
function ler_grupo(&$cha, &$des, &$sta, &$obs) {
     ...
     if ($nro == 0) {
          echo '<script>alert("Código do Grupo de Cliente informado não cadastrado no sistema");</script>';
     } else {
          $cha = $reg['idgrupo'];
